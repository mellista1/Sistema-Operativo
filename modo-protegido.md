...
Modo Protegido: Es el modo operativo nativo del procesador. Brinda un conjunto de características con alto rendimiento, flexibilidad y compatibilidad de sofware ya existente. En este modo se generan direcciones lineales de 32/64 bits. Tiene características diseñadas para soportar la multitarea y la estabilidad del sistema:
    1. Protección de memoria.
    2. Memoria virtual.
    3. Paginación.
Modo Real: Es el modo en el que arrancan todos los x86. Proporciona el entorno de programación del procesador intel 8086. Sus características principales son:
    1. Direcciones físicas de 16 bits.
    2. Sin protección de memoria, es decir, no hay aislamiento de programas.
...
El modo real es un modo de operacion que consiideramos desactualizado ya que no te permite realizar acciones necesarias como proteger la memoria, multitarea, etc. Por este motivo se utiliza, normalmente, el modo protegido. 

La razón por la que no se descontinuo el modo real es que intel desea mantener una retrocompatibilidad con software ya creado.

Si tenemos un sistema operativo en modo real estariamos limitados en las funcionalidades que podriamos tener en el mismo (ya que no podriamos proteger la memoria, como tampoco soportar multitarea y además estaremos limitados en cuanto a la cantidad que nos podemos traer de memoria, entre otras razones).
...
La GDT es una tabla de descriptores de segmento. Cada byte de la misma es accedido con una dirección lógica. Un descriptor de segmento el cual provee al procesador con el tamaño y localizacion de un segmento, como también el control de acceso y los estados de informacion. El formato del descriptor es una estructura de 64 bits.


|  31 - 24|  23  |  22  |  21   |  20  |  19- 16  | 15 |  14 -13  |  12  |  11-8  |  7 - 0  |
|  Base H |  G   |  D/B |   L   |  AVL | SEG LIMIT|  P |  DPL     |  s   |  TYPE  |Base MID |

|    31-16     |       15-0       |
|base 15:0     |segment limit 15:0|

    LIMIT: Especifica el tamaño del segmento.   
    BASE: Define la locación del byte 0 del segmento sin los 4 GBYTES del espacio de la dirección lineal.
    S: Es una flag que indica si el descriptor de segmento es para un segmento de sistema o de código/datos.
    DPL: Especifica el nivel de privilegio del segmento. El nivel puede ir de 0 a 3. 
    P: Es una flag que indica si el segmento esta presente en memoria.
    G: Es una flag que indica la escala del campo de límite del segmento.Cuando G está clear, LIMIT es intrpretado en bytes.
...
> Noten que si bien definimos distintos niveles de privilegio, todos usan la mism.a región de memoria física, por lo cual, no hay una división real de permisos de accesos propuesto a nivel segmentación. Vamos a resolverlo en las próximas clases, cuando agreguemos paginación al kernel.
...
gdt_descriptor_t contiene a la dirección de memoria donde comienza la GDT y, por otro lado, el limite de la misma. 

gdt_entry_t correponde a una entrada de la GDT. Simula a un descriptor de segmento. 
...
Esta instruccion va a inicializar el registro GDTR junto a un puntero que indica la base y además y el límite de la gdt, los cuales los recibe como operando.
En pocas palabras, inicializa nuestra GDT.

En el codigo, la estrcutura que nos indica la dirección y su tamaño es gdt_descriptor_t. 

La inicialización correponde a: Indicar los datos específicos en gdt.c y luego, mediante un extern en kernel.asm, podemos inicializar la tabla correctamente.  
...
Si, hay que modificarlo ya que CR0 contiene la flag p/e que indica si esta en el sistema en modo protegido. 
(1 = modo protegido 0 = real)
...
Usariamos.....(pronto lo sabremos). Vease el siguiente capitulo.
...
Se arma un puntero de tipo p que nos permite acceder a nuestra pantalla. Luego, por cada pixel visitado, se define el caracter y el atributo. Mediante los atributos fInit cInit fSize cSize se define las filas y columnas que se desean acceder en la pantalla

Cada caracter ocupar 2 bytes. El primer byte es el color del caracter y su fondo y el segundo byte es el valor ASCII del caracter.
